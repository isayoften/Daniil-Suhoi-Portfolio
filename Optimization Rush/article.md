...
### 1. Оптимизация памяти и вычислений
#### 1.1. Mixed Precision
...
#### 2.3. Quantization и QLoRa

"Раз fp16 так хорошо работает, может мы еще сильнее сократим точность чисел? До 8 бит или до 4?" Опять таки, в этом вопросе кроется суть квантизации. Правда, опять таки, мы не можем тупо сделать downcast до 8 или 4 бит, это сделает все вычисления крайне нестабильными даже для инференса, не говоря уже об обучении. 

В общем, цель квантизации - понизить потребление памяти, желательно без большой потери точности. Существует много видов квантизации, я расскажу про наиболее простой и распространенный.

Вообще, квантизация может быть применима только для инференса, так как обучение с такой низкой точностью чисел не будет стабильным от слова совсем. (Но мы можем обучать адаптеры поверх квантизованной модели, об этом чуть позже)

Как устроена простейшая квантизация? Посмотрим на рисунок:

![image](https://github.com/user-attachments/assets/49aecbf4-8885-46a5-94e7-139a44a10408)

Просто линейно скейлим параметры в матрицах из fp32 в int8 (или аналогично int4), сохраняя при этом константы, нужные для обратной деквантизации. Таким образом, у нас на руках сжатая в несколько раз (посчитайте сами, во сколько) модель в супер низкой точности, как нам ее поинференсить? Помните про деквантизацию, вы же не думали, что мы будем проводить вычисления прямо в int4? По мере продвижения по слоям, мы деквантизуем матрицы, которые нужны для текущих вычислений, далее считаем промежуточные активации, например в fp16 (используя технику mixed precision), квантизуем матрицу с параметрами обратно, а так же полученные активации, и идем дальше, повторяя этот процесс. 
